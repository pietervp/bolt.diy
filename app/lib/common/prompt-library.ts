...
export class PromptLibrary {
  ...
  > = {
    default: {
      ...
      get: (options) => getSystemPrompt(options.cwd, options.supabase, options.grafx),
    },
    enhanced: {
      label: 'Fine Tuned Prompt',
      description: 'An fine tuned prompt for better results',
      get: (options) => getFineTunedPrompt(options.cwd, options.supabase, options.grafx),
    },
    optimized: {
    ...
    studioUiIntegration: {
      ...
      get: (options) => {
        return optimized(options) + studioUIPrompt;
      },
    },
...
const studioUIPrompt = `# Studio UI Advanced integration
